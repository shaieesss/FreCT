...
class DAC_structure(nn.Module):
    ...
    def forward(self, queries_patch_size, queries_patch_num, keys_patch_size, keys_patch_num, values, patch_index, attn_mask):
        ...
        # 64, 90, 1, 256
        V_patch_size = torch.einsum("bhls,bshd->blhd", series_patch_size, values)
        
        V_patch_num = torch.einsum("bhls,bshd->blhd", series_patch_num, values)
        
        return self.act(V_patch_size), self.act(V_patch_num)
